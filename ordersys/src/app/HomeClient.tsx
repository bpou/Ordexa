...
const QUICK_META: Record<QuickLinkKey, QuickMeta> = {
  ...
  overview: { title: "Alla ordrar", description: "Ã–ppna orderöversikt", href: "/orders/overview", icon: LayoutDashboard },
  ...
  const schedule = useMemo<ScheduleItem[]>(() => {
    ...
    for (const order of scopedOrders) {
      ...
      for (const event of order.events) {
        ...
        if (!group) continue;
        const detail = `${order.customerName ?? "Okänd kund"} Â· ${event.track ? TRACK_NAMES[event.track] : "Delad"}`;
      ...
      for (const track of order.tracks) {
        ...
        const detail = `${order.title} Â· ${order.customerName ?? "Okänd kund"}`;
        const tagMeta = classifyScheduleTag({
          title,
          detail,
          track: track.track,
