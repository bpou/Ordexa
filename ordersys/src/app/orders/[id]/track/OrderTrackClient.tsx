...
// Huvudkomponent
// ============================
export default function OrderTrackClient({ id, track }: { id: string; track: AppTrack }) {
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-xl font-semibold">Order #{id} - Spår {track}</h1>
  ...
  return (
    <div className={`rounded-lg border p-3 shadow-sm ${cardColor}`}>
      <div className="text-sm font-semibold mb-1">
        <Shimmer variant="pill" isLoading={isUpdating} className="inline-block">
          <Link
            className="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold border bg-white/80 hover:bg-white underline decoration-transparent hover:decoration-current transition-all duration-300"
            href={getOrderHref(row.order.orderNumber)}
          >
      ...
      <div className="flex flex-wrap gap-2">
        {(STATI as readonly Status[]).map((s) => {
          ...
          const isUpdatingThis = isUpdating && s === row.status;
          return (
            <Shimmer
              key={s}
              variant="pill"
